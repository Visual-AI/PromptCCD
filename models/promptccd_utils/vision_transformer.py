...
from timm.models._registry import register_model, register_model_deprecations
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, res=None):
        B, nc, w, h = x.shape
        ...
        if res != None:
            ...
            # std_x, mean_x = torch.std_mean(x) # scalars
            # normal_noise = torch.rand(x.shape[0], self.top_k, x.shape[2]).cuda() # (batch_size, top_k, embed_dim)
            # nn.init.normal_(normal_noise, mean=mean_x, std=std_x) # (batch_size, top_k, embed_dim)
            # x = torch.cat([normal_noise, x], dim=1) # (batch_size, top_k + num_patches, embed_dim)
    ...
    def forward(self, x, task_id=-1, res=None):
